...
def load_matting_asset_manifest() -> MattingAssetManifest:
    ...
    content = (
        files("better_backgrounds.matting")
        .joinpath("assets/manifest-v1.json")
...
def verify_packaged_matting_assets() -> MattingAssetManifest:
    ...
    model = files("better_backgrounds.matting").joinpath(
        "assets",
        manifest.model.path,
    )
...
def packaged_seed_model_path() -> Path:
    ...
    resource = files("better_backgrounds.matting").joinpath(
        "assets",
        manifest.model.path,
    )
